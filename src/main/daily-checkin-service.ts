...
import { loadWindowIcon } from './trayIcon'
import {
  AVOIDANCE_FACT,
  AVOIDANCE_THOUGHT
} from '../shared/checkin'
...
export function clearTestReminderOnQuit(): void {
  cancelTestReminder()
  if (fatigueCheckTimer) {
    clearTimeout(fatigueCheckTimer)
...
/** 打开疲劳检查弹窗（URL 附带 mode=fatigue_check） */
export function openFatigueCheckWindow(): void {
  if (checkInWindow && !checkInWindow.isDestroyed()) {
    checkInWindow.focus()
    return
  }
  ...
  const url =
    process.env.ELECTRON_RENDERER_URL ?? `file://${join(__dirname, '../renderer/index.html')}`
  const sep = url.includes('?') ? '&' : '?'
  void checkInWindow.loadURL(`${url}${sep}mode=fatigue_check`)
 ...
 */
export function scheduleFatigueCheck(): void {
  if (fatigueCheckTimer) {
    clearTimeout(fatigueCheckTimer)
    fatigueCheckTimer = null
  }
  ...
  fatigueCheckTimer = setTimeout(() => {
    ...
    fatigueCheckFiredDate = todayStr
    // 压制该时间窗口内可能同时触发的常规提醒
    lastShownAt = Date.now()
