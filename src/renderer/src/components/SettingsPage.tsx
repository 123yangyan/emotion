...
export default function SettingsPage({ onToast }: Props) {
  ...
  const testFatiguePopup = async (): Promise<void> => {
    await window.api.openFatigueCheckPopup()
  }
  ...
  return (
    <div className="settings">
      ...
      <label>
        {ZH.fatigueCheckHourLabel}
        <input
          type="number"
          min={0}
          max={23}
          step={1}
          value={settings.fatigueCheckHour ?? 18}
          onChange={(e) => update('fatigueCheckHour', Number(e.target.value))}
        />
      </label>
      <p className="hint">{ZH.fatigueCheckHourHint}</p>
      <div className="row actions">
        <button type="button" className="btn ghost" onClick={() => void testFatiguePopup()}>
          {ZH.previewFatiguePopup}
        </button>
      </div>
      ...
      <p className="hint">{ZH.exportJsonHint}</p>
