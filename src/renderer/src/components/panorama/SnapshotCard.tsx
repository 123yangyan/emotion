...
export default function SnapshotCard({
  ...
  const hasDiary = point.diaryText.trim().length > 0
  const { tags: thoughtTags, quote: thoughtQuote } = splitThought(
    point.thoughtRaw,
    thoughtTagOptions
  )
  const legacyThought = [...thoughtTags, thoughtQuote].filter(Boolean).join('、')

  const visibleColumns: ContentColumn[] = []
  if (hasDiary) visibleColumns.push('fact')
  if (legacyThought) visibleColumns.push('thought')



  const heroStyle = {

    '--snapshot-hero-color': point.quadrantColor
  ...
  return (
    ...
    >
      ...
      <div className="panorama-snapshot__hero" style={heroStyle}>
        ...
        <span className="panorama-snapshot__hero-emotion">
          {point.coordX > 0 ? `+${point.coordX}` : point.coordX},&nbsp;
          {point.coordY > 0 ? `+${point.coordY}` : point.coordY}
        </span>
        ...
        <div className="panorama-snapshot__grid" style={gridStyle}>
          ...
          {hasDiary ? (
            <div className="panorama-snapshot__col">
              <h4 className="panorama-snapshot__col-title">{ZH.diaryTitle}</h4>
              <p className="panorama-snapshot__supplement">"{point.diaryText}"</p>
            </div>
          ) : null}
          ...
          {legacyThought ? (
            <div className="panorama-snapshot__col">
              ...
              <p className="panorama-snapshot__quote">"{legacyThought}"</p>
            </div>
          ) : null}
