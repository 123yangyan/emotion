...
type ContentColumn = 'fact' | 'thought' | 'body'
...
export default function SnapshotCard({
  ...
  return (
    ...
    >
      ...
      {/* Content Grid：有数据的栏才渲染，高度随内容自适应 */}
      {visibleColumns.length > 0 ? (
        <div className="panorama-snapshot__grid" style={gridStyle}>
          ...
          {hasBody ? (
            <div className="panorama-snapshot__col">
              <h4 className="panorama-snapshot__col-title">{ZH.bodyMind}</h4>
              <div className="chip-wrap panorama-snapshot__chips">
                {point.bodyParts.map((tag) => (
                  <span key={tag} className="chip sm somatic">
                    {tag}
                  </span>
                ))}
              </div>
